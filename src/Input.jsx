...
export function Input(props) {
    ...
    const handleUpdate = (e) => {
        ...
        if (cached === false) {
            ...
            if (val.length > 1 && searchCache.length > 0 && (searchCache[searchCache.length - 1][1] === null || searchCache[searchCache.length - 1][1].length < 1)) {
    ...
    if (roomData) {
        ...
        if (searchResults) {
            res = <div className="z-0 absolute bottom-10 flex flex-col gap-2 w-[220px] sm:w-[500px] text-black bg-[rgb(208,208,228)] text-lg rounded-t-xl overflow-hidden">
                    {
